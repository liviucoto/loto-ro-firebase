...
/// Dialog pentru generarea variantelor bazate pe frecvențe (implementare curată și compactă).
...
class _FrequencyGeneratorDialogState extends State<FrequencyGeneratorDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalDraws = widget.statsDraws.length;
    final topNumbers =
        ...
        Positioned.fill(
          child: Container(
            constraints: BoxConstraints(
              ...
              minWidth: widget.isDesktop ? 600 : 300,
              minHeight: widget.isDesktop ? 400 : 300,
            ),
            child: Material(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // Footer cu statistici frecvență
                          if (generatedVariants.isNotEmpty) ...[
                            const SizedBox(height: 16),
                            _buildFrequencyStatsFooter(),
                          ],
                        ],
                      ),
                    ),
  ...
  Widget _buildStatsCard(bool isMobile, int total, List<int> top, double avg) {
    return Container(
      constraints: BoxConstraints(
        minHeight: 80,
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: Colors.blueAccent.withValues(alpha: 0.10)),
      ),
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                Text('Extrageri: $total', style: AppFonts.captionStyle),
                const SizedBox(width: 10),
                Text('Top 5: ${top.join(', ')}', style: AppFonts.captionStyle),
                const SizedBox(width: 10),
                Text(
                  'Suma medie: ${avg.toStringAsFixed(1)}',
          ...
          Text(
            'Bazat pe frecvența numerelor în perioada selectată',
            style: AppFonts.captionStyle.copyWith(fontSize: 10),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
        ],
  ...
  Widget _buildPlaceholder() => Center(
    child: Text(
      ...
      style: AppFonts.captionStyle.copyWith(color: Colors.blueAccent),
    ),
  );

  Widget _buildFrequencyStatsFooter() {
    final nNumbers = _maxNumberForGame(widget.selectedGame);
    final freq = <int, int>{for (int n = 1; n <= nNumbers; n++) n: 0};
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.analytics, color: Colors.blueAccent, size: 18),
              const SizedBox(width: 8),
              Text(
                'Statistici Frecvență',
                style: AppFonts.bodyStyle.copyWith(
          ...
          const SizedBox(height: 12),
          Text(
            'Top 10 numere frecvente: ${topNumbers.join(', ')}',
            style: AppFonts.captionStyle.copyWith(fontSize: 12),
          ),
          const SizedBox(height: 4),
          Text(
            'Bazat pe ${widget.statsDraws.length} extrageri din perioada selectată',
            style: AppFonts.captionStyle.copyWith(
              fontSize: 11,
  ...
  Future<void> _generateVariantsLocal() async {
    ...
    for (int i = 0; i < nVariants; i++) {
      final variant = _generateSmartVariant(ordered, count, i);
  ...
  String _generateVariantNarrative(dynamic variant, Map<int, int> freq) {
    try {
      ...
      final minFreq = freqs.isNotEmpty
          ? freqs.reduce((a, b) => a < b ? a : b)
          : 0;
  ...
  dynamic _generateSmartVariant(List<int> pool, int count, int variantIndex) {
    final random = Random();
    final nNumbers = _maxNumberForGame(widget.selectedGame);
    final freq = <int, int>{for (int n = 1; n <= nNumbers; n++) n: 0};
    for (final d in widget.statsDraws) {
    ...
    final historic = _getHistoricCombinations();
    final targetEven = count ~/ 2;
    final targetOdd = count - targetEven;
    final variant = <int>[];
    final used = <int>{};
    int even = 0, odd = 0, tries = 0;
    while (variant.length < count && tries < 200) {
      final pick = _weightedRandomPick(freq, random, used);
      if (pick % 2 == 0 && even >= targetEven) {
        tries++;
        continue;
      }
      if (pick % 2 != 0 && odd >= targetOdd) {
        tries++;
        continue;
      }
      final temp = [...variant, pick];
      if (_isHistoricCombination(temp, historic)) {
        tries++;
        continue;
      }
      variant.add(pick);
    ...
    while (variant.length < count) {
      ...
      if (pick % 2 == 0 && even >= targetEven) continue;
      if (pick % 2 != 0 && odd >= targetOdd) continue;
    ...
    if (widget.selectedGame == GameType.joker) {
      ...
      final joker = hasNonZero
          ? _weightedRandomPick(jokerFreq, random, {})
          : 1 + random.nextInt(20);
  ...
  Set<String> _getHistoricCombinations() {
    final set = <String>{};
    for (final d in widget.statsDraws) {
      final sorted = List<int>.from(d.mainNumbers)..sort();
      set.add(sorted.join(','));
    }
    return set;
  }
  ...
  int _weightedRandomPick(Map<int, int> freq, Random random, Set<int> exclude) {
    ...
    // Calculează ponderile - numerele mai frecvente au șanse mai mari
    final maxFreq = entries
        .map((e) => e.value)
        .fold<int>(0, (a, b) => a > b ? a : b);

    // Creează o listă de numere cu ponderi
    final weightedNumbers = <int>[];
    for (final entry in entries) {
      // Ponderea = frecvența + 1 (pentru a evita 0)
      final weight = entry.value + 1;
      // Adaugă numărul de 'weight' ori în listă
      for (int i = 0; i < weight; i++) {
        weightedNumbers.add(entry.key);
      }
    }

    if (weightedNumbers.isEmpty) return entries.first.key;
    return weightedNumbers[random.nextInt(weightedNumbers.length)];
  }
  ...
  void _calculateVariantRatings() {
    ...
    for (final variant in generatedVariants) {
      ...
      final avg = _currentAvgSum();
      final diff = (sum - avg).abs();
      if (diff < 10) {
        rating += 2;
      } else if (diff < 20)
        rating += 1;
      ...
      if (balance <= 1) {
        ...
        rating += 1;
      final hotNums = _currentTopNumbers();
      final hotCount = nums.where((n) => hotNums.contains(n)).length;
      rating += (hotCount / nums.length) * 2;
  ...
  double _currentAvgSum() {
    if (widget.statsDraws.isEmpty) return 0;
    final sums = widget.statsDraws.map(
      (d) => d.mainNumbers.fold(0, (a, b) => a + b),
    );
    return sums.reduce((a, b) => a + b) / sums.length;
  }

  List<int> _currentTopNumbers() {
    if (widget.statsDraws.isEmpty) return [];
    final nNumbers = _maxNumberForGame(widget.selectedGame);
    final freq = <int, int>{for (int n = 1; n <= nNumbers; n++) n: 0};
    for (final d in widget.statsDraws) {
      for (final n in d.mainNumbers) {
